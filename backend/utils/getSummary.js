...
const getSummary = async (transcript) => {
  ...
  const data = {
    ...
    messages: [
      {
        role: "user",
        content: `give me the summary of this clientRecord ${transcript}`,
      },
    ],
  };
...
const getFormattedSummary = async (transcript) => {
  var thread = await openai.beta.threads.create({
    messages: [
      {
        role: "user",
        content: `Make me a formatted summary based on the files you already have and this transcript\n ${transcript}`,
      },
    ],
  });
  console.log(thread);
